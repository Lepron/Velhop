import {Page, Platform, ViewController, NavParams, NavController, Modal, Component, MenuController} from 'ionic-angular';
...
export class HomePage {
  static get parameters() {
    return [[Http],[Platform],[NavController],[NgZone],[MenuController]];
  }

  constructor(http, platform, nav, zone, menu, doml) {
    this.menu=menu;
				...
				for(var i=0;i<this.posts.length;i++){
					...
					google.maps.event.addListener(marker, 'click',(event) => {
						this.zone.run(()=>{
                            ...
                            this.x = this.list[marker.id].go.x;
                            this.y = this.list[marker.id].go.y;
							...
							if(this.list[marker.id].ty!="boutique"){
                                this.id=marker.id;
